...
public class ClientCredentialsServiceTest {

    /**
     * 微信HttpClient <br>
     * 定义HttpClient, 不指定时, 使用默认的HttpClient(RequestFactory.getHttpClient())
     */
    public static HttpClient WECHAT_HTTP_CLIENT = RequestFactory.getHttpClient();
    ...
    @Before
    public void init() {
        // RedissonClient Storage
        // RedissonClient redissonClient = Redisson.create();
        // TokenStorage<WeChatToken> tokenStorage = new WeChatTokenStorage(redissonClient);
        ...
        // InMemoryStorage
        TokenStorage<WeChatToken> tokenStorage = new InMemoryTokenStorage<>();

        // HttpClient
        HttpClient httpClient = WECHAT_HTTP_CLIENT;
        weChatClientCredentialsService = new WeChatClientCredentialsService(clientId, clientSecret, httpClient,
                tokenStorage);
    ...
    /**
     * 微信Token存储 <br>
     * 定义Token存储类, 继承RedissonTokenStorage或实现TokenStorage接口
     */
    public static class WeChatTokenStorage extends RedissonTokenStorage<WeChatToken> {
        public WeChatTokenStorage(RedissonClient redissonClient) {
    ...
    public static class WeChatClientCredentialsService implements ClientCredentialsService<WeChatToken> {
        ...
        /**
         * 不指定时, 使用默认的HttpClient(RequestFactory.getHttpClient())
         */
        private HttpClient httpClient;
        /**
         * 不指定时, 使用默认的Token存储(InMemoryTokenStorage)
         */
        private TokenStorage<WeChatToken> tokenStorage;
        ...
        public WeChatClientCredentialsService(@NonNull String clientId, @NonNull String clientSecret,
                                              @NonNull HttpClient httpClient, @NonNull TokenStorage<WeChatToken> tokenStorage) {
            this.clientId = clientId;
            this.clientSecret = clientSecret;
            this.httpClient = httpClient;
